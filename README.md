...
- 解锁完成后弹窗展示订单号、订单 ID、原价、小票抵扣、积分抵扣和其他优惠
- 解锁完成的订单会保存到本地历史订单，可在“我的”页面查看
- 我的页展示小票、积分、积分抵扣金额
...
随后 App 会展示“订单详情”弹窗，对应原脚本 `afterpay()` 最后打印的订单信息，包括：
...
进入底部导航的“我的”页面，在资产区域下方可以看到“历史订单”。

- 每次设备使用结束并成功查到 `order/detail` 后，订单会自动保存到本地。
- 历史订单最多保留 50 条。
- 点击任意历史订单，可以再次打开订单详情弹窗。
